...
static void i2c_write8(uint32_t i2c, uint8_t address, uint8_t reg8, uint8_t value)
{
...
static void i2c_read_buffer(uint32_t i2c, uint8_t address, uint8_t reg8,
			uint8_t len, uint8_t *buffer)
{
	...
	i2c_send_start_and_wait_for_rdy(i2c);
	/* Acknowledge */
	i2c_send_address_rw_ack(i2c, address, I2C_READ);
...
static uint8_t i2c_read8(uint32_t i2c, uint8_t address, uint8_t reg8)
{
	uint8_t value;
